...
"""
The goal of this file is once 5 seconds of input data are recieved, 
then the intention prediction is called and the predicted intention 
is passed as input to the model trained to predict the future keypoints.
"""
...
def predict(model,int_classifier,motion_input, motion_target, ree_motion_input, ree_motion_target, int_motion_target, forced_intention = None):
    ...
    for idx in range(num_step):
        ...
        with torch.no_grad():
            ...
            if forced_intention is None:
                _, _, int_motion_prediction_ = int_classifier(motion_input_, ree_motion_input_, None)
                intentions.append(int_motion_prediction_.item())
            else:
                int_motion_prediction_ = torch.tensor([forced_intention]) # hard coded intention

            output, int_class_logits,_ = model(motion_input_, ree_motion_input_, int_motion_prediction_.cuda())
